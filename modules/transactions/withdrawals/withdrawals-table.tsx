...
export function WithdrawalsTable() {
  const { t } = useTranslate(["history", "transactions"]);
  ...
  const createDateFromCalendarObject = (calendarObject: any): Date | undefined => {
    if (calendarObject instanceof CalendarDate) {
  ...
  const columns: ColumnDef<Transaction>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-2">
            <span>{fDateTime(transaction.updatedAt)}</span>
          </div>
        );
      },
    },
        ...
        return (
          <div
            className={cn("flex items-center gap-2", {
              "text-destructive": transaction.status === "FAILED",
              "text-amber-500": transaction.status === "PENDING",
              "text-primary": transaction.status === "CONFIRMED",
            })}
          >
            ...
            <span>{t(`operations.status.${transaction.status}`, { ns: "transactions" })}</span>
          </div>
        );
      },
    },
                ...
                <DropdownMenuItem
                  onClick={() =>
                    navigateWithSavedFilters(
                      "withdrawals-table",
                      filters,
                      paths.transactions.withdrawals.details(transaction.id)
                    )
                  }
                >
                  <ReceiptText />
          ...
          >
            <div className="relative w-full md:w-48">
              <SelectTrigger className="w-full pr-10">
                <SelectValue
                  placeholder={t("depositsTable.filters.currencyPlaceholder", {
                    ns: "transactions",
                  })}
                />{" "}
              </SelectTrigger>
